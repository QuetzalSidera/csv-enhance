...
const SHAPED_TYPE_PATTERN_SOURCE = String.raw`(?:(row|col):)?(${TYPE_PATTERN_SOURCE})`;
const FUNC_RETURN_PATTERN_SOURCE = String.raw`(?:(${TYPE_PATTERN_SOURCE})|\[(?:(row|col):)?(${TYPE_PATTERN_SOURCE})\])`;
...
export const FUNCTION_VALUE_PATTERN = new RegExp(
  String.raw`^(${IDENTIFIER_START_PATTERN_SOURCE}${IDENTIFIER_CONTINUE_PATTERN_SOURCE}*)\[(?:${SHAPED_TYPE_PATTERN_SOURCE})\]$`,
  "u",
);
export const FUNC_DIRECTIVE_PATTERN = new RegExp(
  String.raw`^@func\s+(${IDENTIFIER_START_PATTERN_SOURCE}${IDENTIFIER_CONTINUE_PATTERN_SOURCE}*)\s*\((.*)\)\s*=>\s*${FUNC_RETURN_PATTERN_SOURCE}\s*$`,
  "u",
);
